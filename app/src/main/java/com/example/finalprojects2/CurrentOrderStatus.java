...
public class CurrentOrderStatus extends AppCompatActivity {
    ...
    public void checkDeliveryConfirmation(){
        if(TextUtils.isEmpty(custPass.getText().toString()))
            ...
            UserRegister.databaseUsers.addValueEventListener(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    for(DataSnapshot userSnapshot:dataSnapshot.getChildren()){
                        User memberReg=userSnapshot.getValue(User.class);
                        final String name=memberReg.getFullname();
                        final String phone=memberReg.getNo();
